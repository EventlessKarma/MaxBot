...
class RollDatabase:
    ...
    def __init__(self, db_name: str = roll_directory + str(datetime.date.today()) + ".db"):

        print("Opening {}".format(db_name))
    ...
    def get_rolls(self, user_id: int, dice: int):
        ...
        try:
            ...
            print("Error in get_rolls excecute")
            return no_data(dice)
...
def get_all_rolls(user_id: int, dice: int):
    ...
    for file in os.listdir(roll_directory):

        # open the database
        db = RollDatabase(os.path.join(roll_directory,file))
